...
class SearchCollection:
    ...
    def search(self):
        ...
        print("Starting BM25 on all topics")
        ...
        while loop > 0:
            for topic in topics:
                ...
                if self.args.time and loop <= 10:
                    ...
                    rm3 = RM3(self.cursor, query_terms, ranking, 0.8, 0.8, 8)
                    rm_3_terms_str = rm3.rm3()

                    if self.args.disjunctive:
                        sql_query = self.getQueryTemplate(collection_size, avg_doc_len).format(rm_3_terms_str)
                    else:
                        sql_query = self.getQueryTemplate(collection_size, avg_doc_len).format(rm_3_terms_str, 8)

                    self.cursor.execute(sql_query)

            time_end = time.perf_counter()
            print("Done. Time took:")
