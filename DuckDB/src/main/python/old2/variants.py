BM25_STANDARD_TEMPLATE = """
    ...
    SELECT scores.collection_id, scores.id, scores.len, ROUND(score, 6) FROM (SELECT collection_id, id, len, sum(subscore) AS score
        FROM subscores GROUP BY collection_id, id, len) AS scores JOIN docs ON
...
BM25_SHORT_TEMPLATE = """
    ...
    SELECT scores.collection_id, ROUND(score, 6) FROM (SELECT collection_id, sum(subscore) AS score
        FROM subscores GROUP BY collection_id) AS scores JOIN docs ON
        scores.collection_id=docs.collection_id ORDER BY ROUND(score, 6) DESC, scores.collection_id ASC LIMIT 10;
"""
